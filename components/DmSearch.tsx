...
import type { DmMessageWithProfile } from "@/lib/supabase/types";
...
export default function DmSearch({ threadId, partnerName }: Props) {
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState<Tab>("messages");
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<DmMessageWithProfile[]>([]);
  const [links, setLinks] = useState<DmMessageWithProfile[]>([]);
  const [images, setImages] = useState<DmMessageWithProfile[]>([]);
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(60)
      .then(({ data }) => {
        setLinks(((data as DmMessageWithProfile[]) ?? []).filter((m) => extractUrls(m.content).length > 0));
    ...
    supabase
      ...
      .limit(60)
      .then(({ data }) => {
        setImages((data as DmMessageWithProfile[]) ?? []);
  ...
  function handleSearch(val: string) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      const merged: DmMessageWithProfile[] = [];
      for (const m of [...((byContent as DmMessageWithProfile[]) ?? []), ...((byUser as DmMessageWithProfile[]) ?? [])]) {
          ...
          <>
            <div className="search-modal-input-wrap">
              ...
              <input
                ...
                placeholder={`Search messages with ${partnerName}…`}
                value={query}
            ...
            <div className="search-modal-results">
              ...
              {!loading && !query && (
                <div className="search-empty-state">
                  <p className="search-empty-hint">Search your conversation with {partnerName}</p>
                </div>
              )}
        ...
        {tab === "links" && (
          <div className="search-modal-results">
            ...
            {!linksLoading && links.length === 0 && (
              <p className="search-state-msg">No links shared in this conversation yet.</p>
            )}
        ...
        {tab === "images" && (
          <div className="search-modal-results">
            ...
            {!imagesLoading && images.length === 0 && (
              <p className="search-state-msg">No images shared in this conversation yet.</p>
            )}
            {!imagesLoading && images.length > 0 && (
  ...
  return (
    <>
      <button
        ...
        title={`Search messages with ${partnerName}`}
      >
